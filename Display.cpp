...
void Display::processAndPrintString(const String& originalString) {
    uint16_t text_color = TFT_GREEN;
    uint16_t background_color = TFT_BLACK;

    String new_string = originalString;
    if (new_string.startsWith(RED_KEY)) { text_color = TFT_RED;     new_string.remove(0, strlen(RED_KEY)); }
    else if (new_string.startsWith(GREEN_KEY)) { text_color = TFT_GREEN;  new_string.remove(0, strlen(GREEN_KEY)); }
    else if (new_string.startsWith(CYAN_KEY)) { text_color = TFT_CYAN;   new_string.remove(0, strlen(CYAN_KEY)); }
    else if (new_string.startsWith(WHITE_KEY)) { text_color = TFT_WHITE;  new_string.remove(0, strlen(WHITE_KEY)); }
    else if (new_string.startsWith(MAGENTA_KEY)) { text_color = TFT_MAGENTA;new_string.remove(0, strlen(MAGENTA_KEY)); }

    this->tft.setTextColor(text_color, background_color);
    this->tft.print(new_string);
}
  ...
  if (this->display_buffer->size() > 0)
  {
    ...
    while ((display_buffer->size() > 0) && (print_count > 0))
    {
        ...
        for (int i = 0; i < this->screen_buffer->size(); i++) {
          ...
          this->processAndPrintString(this->screen_buffer->get(i));
        }
      #endif
